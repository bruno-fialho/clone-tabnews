  ...
  describe("Anonymous user", () => {
    test("With incorrect `email` but correct `password`", async () => {
      ...
      const response = await fetch("http://localhost:3000/api/v1/sessions", {
        ...
        body: JSON.stringify({
          email: "email.incorrecto@email.com",
          password: "senha_correta",
        }),
      });
    ...
    test("With correct `email` but correct `password`", async () => {
      ...
      const diffInMilliseconds = expiresAt - createdAt;

      // Allow a small tolerance (±1 second) to account for timing differences
      const tolerance = 1000;
      expect(diffInMilliseconds).toBeGreaterThanOrEqual(
        session.EXPIRATION_IN_MILLISECONDS - tolerance,
      );
      expect(diffInMilliseconds).toBeLessThanOrEqual(
        session.EXPIRATION_IN_MILLISECONDS + tolerance,
      );
